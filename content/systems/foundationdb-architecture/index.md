...
The master has appeared twice in this path, and it is worth closing with what it actually does, because it is less than its name suggests. It hands out commit versions to the commit proxies — one monotonically increasing sequence. Commit versions establish the global serialization order. The master also keeps track of how far that sequence has become durable. Assigning a commit version is not committing: after the master hands one out, the transaction still has to pass the resolvers and reach the transaction logs, and it may abort along the way. So each commit proxy, once its batch is durable in the logs, reports the version back to the master before replying to the client. The master keeps the highest durable version reported so far, called the live committed version, and that — not the latest version it has assigned — is what it returns to a GRV proxy asking for a read version. A read version therefore never covers a transaction that is still committing and not yet durable, and it always covers every transaction whose commit has been acknowledged to its client (a durable transaction), which is the property behind the strict serializability guarantee above. The master is a sequencer, and keeping it that small — with the proxies absorbing all the client traffic — is what lets a single process sit in the path of every transaction without becoming the bottleneck.
